output "manifest" {
  ...
  value = {
    ...
    alb = {
      ...
      # Real, provider-generated ALB identity. Informational only — never
      # resolvable from the verifier's network namespace in this
      # environment. See contracts/architecture.md.
      dns_name       = aws_lb.main.dns_name
      advertised_url = "http://${aws_lb.main.dns_name}"
      # connect_url is what the verifier (and this script's own readiness
      # poll) actually calls. It MUST be the shared AWS-compatible endpoint,
      # not the real ALB DNS name above — do not collapse these two
      # concepts into one value.
