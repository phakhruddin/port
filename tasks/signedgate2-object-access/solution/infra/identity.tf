...
resource "aws_cognito_user_pool" "main" {
  ...
  # This configuration never sets these blocks, so any value the emulator
  # reports for them is provider-side noise, not drift we intend to
  # reconcile. Left unmanaged, that noise can produce an incidental
  # UpdateUserPool call during an unrelated `apply` (for example, during
  # endpoint-repair recovery), and the emulator rejects such calls unless
  # they explicitly carry UserPoolAddOns.AdvancedSecurityMode. Ignoring
  # these fields keeps every `apply` scoped to changes this configuration
  # actually intends. See contracts/services/identity.md's "Emulator note".
  lifecycle {
    ignore_changes = [
      email_configuration,
