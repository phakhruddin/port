...
umask 077
# Every dynamic value Terraform needs is persisted here, not passed via -var
# flags on `apply` below. The verifier runs `terraform plan` directly against
# infra/, without going through this script, so an auto-loaded tfvars file is
# the only way that standalone plan has what it needs. See
# contracts/architecture.md's "Shared rules" and reasoning.md's "What changed
# from v1" for why this is called out explicitly.
...
# alb.connect_url (not alb.dns_name / alb.url) is the endpoint this script
# and the verifier both use to reach SignedGate. See
# contracts/architecture.md's "The manifest's alb object" section.
