...
| Oracle (downloaded artifacts) | 0 of 3 | — | — | — | All three were platform/runtime startup failures, not solution failures |
...
view is more informative — e.g., within the `90812a88…` digest cohort,
Gemini's 81.82 beat Claude's 63.64 and GPT's 9.09; within the `f17df02b…`
cohort, Claude's 72.73 led both others at 63.64.
...
2. **Invalid Cognito token endpoint** (2 runs). `token_url` didn't match the
   ...
   `/<pool-id>/oauth2/token` variants observed) → HTTP 400 during credential
   setup → every behavioral test errors acquiring a token.
...
- **The single highest-leverage detail across all three models was the
  manifest's connect URL.** Getting the emulator-reachable vs.
